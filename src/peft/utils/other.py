...
class Sin(nn.Module):
    ...
    def __init__(self, A: float = 0.0005, omega: float = 1000) -> None:
        super().__init__()
        self.A = A
        self.omega = omega

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return self.A * input * torch.sin(self.omega * input) + 1.0 * input


class Cos(nn.Module):
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return 1.0 * input * torch.cos(10000.0 * input) + 1.0 * input


class Gaussian(nn.Module):
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return 0.5 * input * torch.exp(-(input ** 2))
